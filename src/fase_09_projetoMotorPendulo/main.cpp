#include <Arduino.h>
#include <Wire.h>
#include <AS5600.h>

#include <PendulumPosition.h>
#include <PendulumVelocity.h>
#include <MotorVelocity.h>

#include <math.h>
#include <ctype.h>
#include <string.h>


// ============================================================
// FASE 09 — CONTROLE CONJUNTO DE QUATRO ESTADOS
// Pendulo de Furuta
// ============================================================
//
// Estado:
//
//      x = [ phi, phiDot, beta, betaDot ]^T
//
// onde:
//
//      phi      = posicao do braco [rad]
//      phiDot   = velocidade comandada do braco [rad/s]
//      beta     = angulo do pendulo em torno da vertical [rad]
//      betaDot  = velocidade angular estimada do pendulo [rad/s]
//
// ------------------------------------------------------------
//
// MODELO AUMENTADO:
//
//      phiDDot = u
//
//      betaDDot =
//          21.7*beta
//        - 3.28*betaDot
//        - 0.75*u
//
// ------------------------------------------------------------
//
// POLos DESEJADOS:
//
//      -0.7
//      -1.0
//      -4.0
//      -6.0
//
// ------------------------------------------------------------
//
// REALIMENTACAO DE ESTADOS:
//
// MATLAB:
//
//      u = -K*x
//
//      K = [
//          -0.77419
//          -2.31979
//          -95.71079
//          -14.31971
//      ]
//
// Portanto, na nossa convencao:
//
//      u =
//          +0.77419*phi
//          +2.31979*phiDot
//          +95.71079*beta
//          +14.31971*betaDot
//
// IMPORTANTE:
//
// Nao alterar intuitivamente os sinais dos termos de phi.
// Os quatro ganhos foram calculados CONJUNTAMENTE.
//
// ------------------------------------------------------------
//
// REFERENCIA DO PENDULO:
//
// T deve ser executado com o pendulo:
//
//      LIVRE
//      PARADO
//      PARA BAIXO
//
// A vertical superior e calculada como:
//
//      TOP = DOWN + 180 graus
//
// Assim:
//
//      beta = 0        -> vertical superior
//      beta = +/-PI    -> vertical inferior
//
// ------------------------------------------------------------
//
// CAPTURA:
//
//      READY:
//          |beta| <= 1.5 graus
//          |betaDot| <= 0.25 rad/s
//          durante 80 ms
//
//      RELEASE:
//          |betaDot| >= 0.05 rad/s
//
//      CAPTURE_LOST:
//          |beta| > 1.75 graus
//
// ------------------------------------------------------------
//
// PRIMEIRO ENSAIO:
//
//      Ts             = 4 ms
//      u max          = +/-600 deg/s2
//      beta abort     = +/-8 graus
//      phi abort      = +/-75 graus
//      phi absoluto   = +/-80 graus
//      tempo maximo   = 5 s
//
// ============================================================


// ============================================================
// SERIAL
// ============================================================

constexpr uint32_t SERIAL_BAUD =
    250000UL;


// ============================================================
// HARDWARE
// ============================================================

constexpr uint8_t STEP_PIN =
    6;

constexpr uint8_t DIR_PIN =
    8;

constexpr uint8_t ENABLE_PIN =
    4;


// ============================================================
// MOTOR
// ============================================================

constexpr uint16_t FULL_STEPS_PER_REVOLUTION =
    200;

constexpr uint8_t MICROSTEP_FACTOR =
    8;


// Limites absolutos conhecidos pelo MotorVelocity.

constexpr float ARM_MINIMUM_DEGREES =
    -80.0F;

constexpr float ARM_MAXIMUM_DEGREES =
    80.0F;


// Velocidade maxima ja validada experimentalmente.

constexpr float MOTOR_MAX_SPEED_DEG_S =
    180.0F;


// ============================================================
// TEMPORIZACAO
// ============================================================

constexpr uint32_t CONTROL_PERIOD_US =
    4000UL;


constexpr uint32_t MAX_CONTROL_DT_US =
    20000UL;


// Primeiro ensaio limitado a 5 segundos.

constexpr uint32_t BALANCE_TEST_TIME_US =
    100000000UL;


// ============================================================
// CONVERSOES
// ============================================================

constexpr float DEG_TO_RAD_LOCAL =
    0.01745329251994329577F;

constexpr float RAD_TO_DEG_LOCAL =
    57.295779513082320876F;

constexpr float PI_LOCAL =
    3.14159265358979323846F;

constexpr float TWO_PI_LOCAL =
    6.28318530717958647692F;


// ============================================================
// MODELO IDENTIFICADO
// ============================================================
//
// betaDDot =
//      MODEL_A * beta
//    + MODEL_B * betaDot
//    + MODEL_C * u
//
// ============================================================

constexpr float MODEL_A =
    21.7F;

constexpr float MODEL_B =
    -3.28F;

constexpr float MODEL_C =
    -0.75F;


// ============================================================
// CONTROLE DE ESTADOS
// ============================================================
//
// Resultado do posicionamento conjunto de polos:
//
//      p = [-0.7, -1.0, -4.0, -6.0]
//
// Usamos diretamente:
//
// u =
//      K_PHI      * phi
//    + K_PHI_DOT  * phiDot
//    + K_BETA     * beta
//    + K_BETA_DOT * betaDot
//
// ============================================================

constexpr float K_PHI =
    0.77419355F;


constexpr float K_PHI_DOT =
    2.31978594F;


constexpr float K_BETA =
    95.71078856F;


constexpr float K_BETA_DOT =
    14.31971458F;


// ============================================================
// SATURACAO DA ACELERACAO
// ============================================================

constexpr float CONTROL_MAX_ACCEL_DEG_S2 =
    600.0F;


constexpr float CONTROL_MAX_ACCEL_RAD_S2 =
    CONTROL_MAX_ACCEL_DEG_S2
    *
    DEG_TO_RAD_LOCAL;


// ============================================================
// CAPTURA
// ============================================================

constexpr float CAPTURE_READY_BETA_DEG =
    1.5F;


constexpr float CAPTURE_READY_BETA_RAD =
    CAPTURE_READY_BETA_DEG
    *
    DEG_TO_RAD_LOCAL;


constexpr float CAPTURE_READY_BETADOT_RAD_S =
    0.25F;


constexpr uint32_t CAPTURE_READY_TIME_US =
    80000UL;


constexpr float RELEASE_VELOCITY_RAD_S =
    0.05F;


constexpr float CAPTURE_RELEASE_BETA_DEG =
    1.75F;


constexpr float CAPTURE_RELEASE_BETA_RAD =
    CAPTURE_RELEASE_BETA_DEG
    *
    DEG_TO_RAD_LOCAL;


// ============================================================
// LIMITES DE SEGURANCA
// ============================================================

constexpr float BETA_ABORT_DEG =
    8.0F;


constexpr float BETA_ABORT_RAD =
    BETA_ABORT_DEG
    *
    DEG_TO_RAD_LOCAL;


// Limite operacional.
//
// Deixa 5 graus de margem em relacao aos +/-80 graus
// absolutos do MotorVelocity.

constexpr float ARM_BALANCE_LIMIT_DEG =
    75.0F;


// ============================================================
// SENSOR
// ============================================================

AS5600 as5600;


PendulumPosition pendulumPosition(
    as5600,
    1.0F
);


PendulumVelocity pendulumVelocity;


// ============================================================
// MOTOR
// ============================================================

MotorVelocity motor(
    STEP_PIN,
    DIR_PIN,
    ENABLE_PIN,

    FULL_STEPS_PER_REVOLUTION,
    MICROSTEP_FACTOR,

    ARM_MINIMUM_DEGREES,
    ARM_MAXIMUM_DEGREES,

    false
);


// ============================================================
// ESTADOS DA MAQUINA
// ============================================================

enum class ControlState
{
    IDLE,

    WAIT_CAPTURE,

    WAIT_RELEASE,

    BALANCE
};


ControlState controlState =
    ControlState::IDLE;


// ============================================================
// REFERENCIAS
// ============================================================

bool armZeroDefined =
    false;


bool pendulumDownReferenceDefined =
    false;


// ============================================================
// TEMPORIZACAO
// ============================================================

uint32_t nextControlTimeUs =
    0;


uint32_t lastControlTimeUs =
    0;


uint32_t captureStableStartUs =
    0;


uint32_t balanceStartUs =
    0;


// ============================================================
// ESTADOS
// ============================================================

// Pendulo

float betaRadians =
    0.0F;


float betaDotRadiansPerSecond =
    0.0F;


// Braco

float phiDegrees =
    0.0F;


float phiRadians =
    0.0F;


float phiDotCommandDegS =
    0.0F;


float phiDotCommandRadS =
    0.0F;


// ============================================================
// COMPONENTES DO CONTROLE
// ============================================================

float controlPhiRadS2 =
    0.0F;


float controlPhiDotRadS2 =
    0.0F;


float controlBetaRadS2 =
    0.0F;


float controlBetaDotRadS2 =
    0.0F;


// ============================================================
// CONTROLE TOTAL
// ============================================================

float controlRawRadS2 =
    0.0F;


float controlLimitedRadS2 =
    0.0F;


float controlCommandDegS2 =
    0.0F;


// ============================================================
// DIAGNOSTICOS
// ============================================================

float maximumAbsBetaRad =
    0.0F;


float maximumAbsBetaDotRadS =
    0.0F;


float maximumAbsPhiDeg =
    0.0F;


float maximumAbsPhiDotDegS =
    0.0F;


float maximumAbsControlDegS2 =
    0.0F;


// Maior valor absoluto de cada parcela do controlador.

float maximumAbsPhiTermDegS2 =
    0.0F;


float maximumAbsPhiDotTermDegS2 =
    0.0F;


float maximumAbsBetaTermDegS2 =
    0.0F;


float maximumAbsBetaDotTermDegS2 =
    0.0F;


uint32_t saturationCounter =
    0;


uint32_t controlSampleCounter =
    0;


// ============================================================
// SNAPSHOT FINAL
// ============================================================
//
// Alguns valores precisam ser guardados ANTES de motor.stop(),
// principalmente phiDot.
//
// ============================================================

float finalPhiDeg =
    0.0F;


float finalPhiDotCommandDegS =
    0.0F;


float finalControlCommandDegS2 =
    0.0F;


uint32_t finalBalanceElapsedUs =
    0;


// ============================================================
// SERIAL
// ============================================================

constexpr uint8_t SERIAL_BUFFER_SIZE =
    24;


char serialBuffer[
    SERIAL_BUFFER_SIZE
];


uint8_t serialBufferIndex =
    0;


// ============================================================
// PROTOTIPOS
// ============================================================

float wrapToPi(
    float angle
);


float calculateBetaFromDownReference();


float limitFloat(
    float value,
    float minimum,
    float maximum
);


void serviceSerial();


void handleCommand(
    char *command
);


void controlTick(
    uint32_t nowUs
);


void serviceWaitCapture(
    uint32_t nowUs
);


void serviceWaitRelease(
    uint32_t nowUs
);


void startBalance(
    uint32_t nowUs
);


void serviceBalance(
    uint32_t nowUs,
    float dtSeconds,
    uint32_t dtUs
);


void endBalance(
    const char *reason
);


void stopControl();


void clearControlVariables();


void definePendulumDownReference();


void defineArmZero();


void armBalance();


void emergencyStop();


void printStatus();


void printHelp();


void printBalanceSummary(
    const char *reason
);


const char *stateName();


// ============================================================
// WRAP ANGULAR
// ============================================================

float wrapToPi(
    float angle
)
{
    while (
        angle >
        PI_LOCAL
    )
    {
        angle -=
            TWO_PI_LOCAL;
    }


    while (
        angle <=
        -PI_LOCAL
    )
    {
        angle +=
            TWO_PI_LOCAL;
    }


    return angle;
}


// ============================================================
// BETA REFERENCIADO AO TOPO
// ============================================================
//
// A biblioteca PendulumPosition sera calibrada com o pendulo
// PARA BAIXO.
//
// Portanto:
//
//      beta_library = 0
//
// corresponde a DOWN.
//
// Para obter beta=0 no topo:
//
//      beta = wrap(beta_library - PI)
//
// ============================================================

float calculateBetaFromDownReference()
{
    if (
        !pendulumDownReferenceDefined
    )
    {
        return 0.0F;
    }


    const float betaFromDown =
        pendulumPosition.betaRadians();


    return wrapToPi(
        betaFromDown
        -
        PI_LOCAL
    );
}


// ============================================================
// LIMITADOR
// ============================================================

float limitFloat(
    float value,
    float minimum,
    float maximum
)
{
    if (
        value >
        maximum
    )
    {
        return maximum;
    }


    if (
        value <
        minimum
    )
    {
        return minimum;
    }


    return value;
}


// ============================================================
// LIMPAR VARIAVEIS DO CONTROLADOR
// ============================================================

void clearControlVariables()
{
    controlPhiRadS2 =
        0.0F;


    controlPhiDotRadS2 =
        0.0F;


    controlBetaRadS2 =
        0.0F;


    controlBetaDotRadS2 =
        0.0F;


    controlRawRadS2 =
        0.0F;


    controlLimitedRadS2 =
        0.0F;


    controlCommandDegS2 =
        0.0F;
}


// ============================================================
// SETUP
// ============================================================

void setup()
{
    Serial.begin(
        SERIAL_BAUD
    );


    Wire.begin();


    Wire.setClock(
        400000UL
    );


    // --------------------------------------------------------
    // SENSOR
    // --------------------------------------------------------

    pendulumPosition.begin();


    pendulumPosition.update();


    const uint32_t nowUs =
        micros();


    betaRadians =
        0.0F;


    betaDotRadiansPerSecond =
        0.0F;


    pendulumVelocity.reset(
        betaRadians,
        nowUs
    );


    // --------------------------------------------------------
    // MOTOR
    // --------------------------------------------------------

    motor.begin(
        MOTOR_MAX_SPEED_DEG_S,
        2
    );


    armZeroDefined =
        false;


    pendulumDownReferenceDefined =
        false;


    // --------------------------------------------------------
    // CONTROLE
    // --------------------------------------------------------

    clearControlVariables();


    // --------------------------------------------------------
    // TIMING
    // --------------------------------------------------------

    lastControlTimeUs =
        micros();


    nextControlTimeUs =
        lastControlTimeUs
        +
        CONTROL_PERIOD_US;


    // --------------------------------------------------------
    // CABECALHO
    // --------------------------------------------------------

    Serial.println();


    Serial.println(
        F("========================================")
    );


    Serial.println(
        F("FASE 09 - CONTROLE DE 4 ESTADOS")
    );


    Serial.println(
        F("Pendulo de Furuta")
    );


    Serial.println(
        F("========================================")
    );


    Serial.println();


    Serial.println(
        F("ESTADO:")
    );


    Serial.println(
        F("x = [phi phiDot beta betaDot]")
    );


    Serial.println();


    Serial.println(
        F("MODELO:")
    );


    Serial.println(
        F("phiDDot = u")
    );


    Serial.println(
        F("betaDDot = 21.7*beta -3.28*betaDot -0.75*u")
    );


    Serial.println();


    Serial.println(
        F("POLOS:")
    );


    Serial.println(
        F("-0.7  -1.0  -4.0  -6.0")
    );


    Serial.println();


    Serial.println(
        F("CONTROLE:")
    );


    Serial.println(
        F("u = Kphi*phi + KphiDot*phiDot")
    );


    Serial.println(
        F("  + Kbeta*beta + KbetaDot*betaDot")
    );


    Serial.println();


    Serial.print(
        F("Kphi = ")
    );

    Serial.println(
        K_PHI,
        5
    );


    Serial.print(
        F("KphiDot = ")
    );

    Serial.println(
        K_PHI_DOT,
        5
    );


    Serial.print(
        F("Kbeta = ")
    );

    Serial.println(
        K_BETA,
        5
    );


    Serial.print(
        F("KbetaDot = ")
    );

    Serial.println(
        K_BETA_DOT,
        5
    );


    Serial.println();


    Serial.println(
        F("LIMITES:")
    );


    Serial.println(
        F("Ts = 4 ms")
    );


    Serial.println(
        F("u = +/-600 deg/s2")
    );


    Serial.println(
        F("beta abort = +/-8 deg")
    );


    Serial.println(
        F("phi abort = +/-75 deg")
    );


    Serial.println(
        F("phi absoluto = +/-80 deg")
    );


    Serial.println(
        F("teste inicial = 5 s")
    );


    Serial.println();


    Serial.println(
        F("REFERENCIA:")
    );


    Serial.println(
        F("T = pendulo LIVRE, PARADO e PARA BAIXO.")
    );


    Serial.println(
        F("TOP = DOWN + 180 graus.")
    );


    Serial.println();


    Serial.println(
        F("Motor iniciado DESABILITADO.")
    );


    if (
        pendulumPosition.magnetDetected()
    )
    {
        Serial.println(
            F("AS5600: ima detectado.")
        );
    }
    else
    {
        Serial.println(
            F("ATENCAO: ima nao detectado.")
        );
    }


    Serial.println();


    printHelp();
}


// ============================================================
// LOOP
// ============================================================

void loop()
{
    // runSpeed precisa ser chamado frequentemente.

    motor.update();


    serviceSerial();


    motor.update();


    const uint32_t nowUs =
        micros();


    if (
        static_cast<int32_t>(
            nowUs
            -
            nextControlTimeUs
        )
        >=
        0
    )
    {
        controlTick(
            nowUs
        );


        nextControlTimeUs +=
            CONTROL_PERIOD_US;


        // Se algum processamento atrasou muito,
        // nao tentamos executar varios ciclos acumulados.

        if (
            static_cast<int32_t>(
                micros()
                -
                nextControlTimeUs
            )
            >=
            0
        )
        {
            nextControlTimeUs =
                micros()
                +
                CONTROL_PERIOD_US;
        }
    }


    motor.update();
}


// ============================================================
// TICK PRINCIPAL
// ============================================================

void controlTick(
    uint32_t nowUs
)
{
    const uint32_t dtUs =
        nowUs
        -
        lastControlTimeUs;


    const float dtSeconds =
        static_cast<float>(
            dtUs
        )
        *
        1.0e-6F;


    lastControlTimeUs =
        nowUs;


    // --------------------------------------------------------
    // SENSOR
    // --------------------------------------------------------

    pendulumPosition.update();


    if (
        pendulumDownReferenceDefined
    )
    {
        betaRadians =
            calculateBetaFromDownReference();


        pendulumVelocity.update(
            betaRadians,
            nowUs
        );


        if (
            pendulumVelocity.isReady()
        )
        {
            betaDotRadiansPerSecond =
                pendulumVelocity.radiansPerSecond();
        }
    }


    // --------------------------------------------------------
    // MAQUINA DE ESTADOS
    // --------------------------------------------------------

    switch (
        controlState
    )
    {
        case ControlState::IDLE:
        {
            break;
        }


        case ControlState::WAIT_CAPTURE:
        {
            serviceWaitCapture(
                nowUs
            );

            break;
        }


        case ControlState::WAIT_RELEASE:
        {
            serviceWaitRelease(
                nowUs
            );

            break;
        }


        case ControlState::BALANCE:
        {
            serviceBalance(
                nowUs,
                dtSeconds,
                dtUs
            );

            break;
        }
    }
}


// ============================================================
// WAIT_CAPTURE
// ============================================================

void serviceWaitCapture(
    uint32_t nowUs
)
{
    if (
        !pendulumVelocity.isReady()
    )
    {
        captureStableStartUs =
            0;


        return;
    }


    const bool betaOK =
        fabsf(
            betaRadians
        )
        <=
        CAPTURE_READY_BETA_RAD;


    const bool betaDotOK =
        fabsf(
            betaDotRadiansPerSecond
        )
        <=
        CAPTURE_READY_BETADOT_RAD_S;


    const bool phiOK =
        fabsf(
            motor.currentPositionDegrees()
        )
        <=
        5.0F;


    if (
        betaOK
        &&
        betaDotOK
        &&
        phiOK
    )
    {
        if (
            captureStableStartUs
            ==
            0
        )
        {
            captureStableStartUs =
                nowUs;
        }


        if (
            (
                nowUs
                -
                captureStableStartUs
            )
            >=
            CAPTURE_READY_TIME_US
        )
        {
            controlState =
                ControlState::WAIT_RELEASE;


            captureStableStartUs =
                0;


            Serial.println();


            Serial.println(
                F("CAPTURE_READY")
            );


            Serial.print(
                F("beta [deg] = ")
            );

            Serial.println(
                betaRadians
                *
                RAD_TO_DEG_LOCAL,
                3
            );


            Serial.print(
                F("betaDot [rad/s] = ")
            );

            Serial.println(
                betaDotRadiansPerSecond,
                4
            );


            Serial.println();


            Serial.println(
                F("SOLTE O PENDULO.")
            );


            Serial.println();
        }
    }
    else
    {
        captureStableStartUs =
            0;
    }
}


// ============================================================
// WAIT_RELEASE
// ============================================================

void serviceWaitRelease(
    uint32_t nowUs
)
{
    if (
        !pendulumVelocity.isReady()
    )
    {
        return;
    }


    const float absBeta =
        fabsf(
            betaRadians
        );


    // Saiu demais do topo antes de detectar a soltura.

    if (
        absBeta
        >
        CAPTURE_RELEASE_BETA_RAD
    )
    {
        motor.stop();


        controlState =
            ControlState::WAIT_CAPTURE;


        captureStableStartUs =
            0;


        Serial.println();


        Serial.println(
            F("CAPTURE_LOST")
        );


        Serial.print(
            F("beta [deg] = ")
        );

        Serial.println(
            betaRadians
            *
            RAD_TO_DEG_LOCAL,
            3
        );


        Serial.println(
            F("Reposicione perto da vertical.")
        );


        Serial.println();


        return;
    }


    // Soltura detectada.

    if (
        fabsf(
            betaDotRadiansPerSecond
        )
        >=
        RELEASE_VELOCITY_RAD_S
    )
    {
        startBalance(
            nowUs
        );
    }
}


// ============================================================
// START BALANCE
// ============================================================

void startBalance(
    uint32_t nowUs
)
{
    motor.stop();


    clearControlVariables();


    // --------------------------------------------------------
    // ESTATISTICAS
    // --------------------------------------------------------

    maximumAbsBetaRad =
        fabsf(
            betaRadians
        );


    maximumAbsBetaDotRadS =
        fabsf(
            betaDotRadiansPerSecond
        );


    maximumAbsPhiDeg =
        fabsf(
            motor.currentPositionDegrees()
        );


    maximumAbsPhiDotDegS =
        fabsf(
            motor.speedReferenceDegreesPerSecond()
        );


    maximumAbsControlDegS2 =
        0.0F;


    maximumAbsPhiTermDegS2 =
        0.0F;


    maximumAbsPhiDotTermDegS2 =
        0.0F;


    maximumAbsBetaTermDegS2 =
        0.0F;


    maximumAbsBetaDotTermDegS2 =
        0.0F;


    saturationCounter =
        0;


    controlSampleCounter =
        0;


    // --------------------------------------------------------
    // CABECALHO
    // --------------------------------------------------------

    Serial.println();


    Serial.println(
        F("========================================")
    );


    Serial.println(
        F("BALANCE - CONTROLE DE 4 ESTADOS")
    );


    Serial.println(
        F("========================================")
    );


    Serial.print(
        F("beta inicial [deg] = ")
    );

    Serial.println(
        betaRadians
        *
        RAD_TO_DEG_LOCAL,
        3
    );


    Serial.print(
        F("betaDot inicial [rad/s] = ")
    );

    Serial.println(
        betaDotRadiansPerSecond,
        4
    );


    Serial.print(
        F("phi inicial [deg] = ")
    );

    Serial.println(
        motor.currentPositionDegrees(),
        3
    );


    Serial.print(
        F("phiDot inicial [deg/s] = ")
    );

    Serial.println(
        motor.speedReferenceDegreesPerSecond(),
        3
    );


    Serial.println();


    // --------------------------------------------------------
    // IMPORTANTE:
    //
    // O tempo do ensaio comeca somente DEPOIS das mensagens.
    // --------------------------------------------------------

    const uint32_t startUs =
        micros();


    balanceStartUs =
        startUs;


    lastControlTimeUs =
        startUs;


    nextControlTimeUs =
        startUs
        +
        CONTROL_PERIOD_US;


    controlState =
        ControlState::BALANCE;
}


// ============================================================
// BALANCE
// ============================================================

void serviceBalance(
    uint32_t nowUs,
    float dtSeconds,
    uint32_t dtUs
)
{
    // ========================================================
    // TIMING
    // ========================================================

    if (
        dtUs
        >
        MAX_CONTROL_DT_US
    )
    {
        endBalance(
            "CONTROL_OVERRUN"
        );


        return;
    }


    // ========================================================
    // VELOCIDADE DO PENDULO
    // ========================================================

    if (
        !pendulumVelocity.isReady()
    )
    {
        endBalance(
            "VELOCITY_NOT_READY"
        );


        return;
    }


    // ========================================================
    // ESTADOS DO BRACO
    // ========================================================

    phiDegrees =
        motor.currentPositionDegrees();


    phiRadians =
        phiDegrees
        *
        DEG_TO_RAD_LOCAL;


    phiDotCommandDegS =
        motor.speedReferenceDegreesPerSecond();


    phiDotCommandRadS =
        phiDotCommandDegS
        *
        DEG_TO_RAD_LOCAL;


    // ========================================================
    // VALORES ABSOLUTOS
    // ========================================================

    const float absBeta =
        fabsf(
            betaRadians
        );


    const float absBetaDot =
        fabsf(
            betaDotRadiansPerSecond
        );


    const float absPhiDeg =
        fabsf(
            phiDegrees
        );


    const float absPhiDotDegS =
        fabsf(
            phiDotCommandDegS
        );


    // ========================================================
    // ESTATISTICAS DOS ESTADOS
    // ========================================================

    if (
        absBeta
        >
        maximumAbsBetaRad
    )
    {
        maximumAbsBetaRad =
            absBeta;
    }


    if (
        absBetaDot
        >
        maximumAbsBetaDotRadS
    )
    {
        maximumAbsBetaDotRadS =
            absBetaDot;
    }


    if (
        absPhiDeg
        >
        maximumAbsPhiDeg
    )
    {
        maximumAbsPhiDeg =
            absPhiDeg;
    }


    if (
        absPhiDotDegS
        >
        maximumAbsPhiDotDegS
    )
    {
        maximumAbsPhiDotDegS =
            absPhiDotDegS;
    }


    // ========================================================
    // LIMITES DE SEGURANCA
    // ========================================================

    if (
        absBeta
        >=
        BETA_ABORT_RAD
    )
    {
        endBalance(
            "BETA_LIMIT"
        );


        return;
    }


    if (
        absPhiDeg
        >=
        ARM_BALANCE_LIMIT_DEG
    )
    {
        endBalance(
            "ARM_LIMIT"
        );


        return;
    }


    // ========================================================
    // TEMPO MAXIMO
    // ========================================================

    if (
        (
            nowUs
            -
            balanceStartUs
        )
        >=
        BALANCE_TEST_TIME_US
    )
    {
        endBalance(
            "TIME_LIMIT"
        );


        return;
    }


    // ========================================================
    // CONTROLE DE QUATRO ESTADOS
    // ========================================================
    //
    // u =
    //      Kphi      * phi
    //    + KphiDot   * phiDot
    //    + Kbeta     * beta
    //    + KbetaDot  * betaDot
    //
    // Todas as variaveis em unidades SI.
    //
    // ========================================================

    controlPhiRadS2 =
        K_PHI
        *
        phiRadians;


    controlPhiDotRadS2 =
        K_PHI_DOT
        *
        phiDotCommandRadS;


    controlBetaRadS2 =
        K_BETA
        *
        betaRadians;


    controlBetaDotRadS2 =
        K_BETA_DOT
        *
        betaDotRadiansPerSecond;


    controlRawRadS2 =
        controlPhiRadS2
        +
        controlPhiDotRadS2
        +
        controlBetaRadS2
        +
        controlBetaDotRadS2;


    // ========================================================
    // SATURACAO
    // ========================================================

    controlLimitedRadS2 =
        limitFloat(
            controlRawRadS2,
            -CONTROL_MAX_ACCEL_RAD_S2,
            CONTROL_MAX_ACCEL_RAD_S2
        );


    if (
        fabsf(
            controlRawRadS2
            -
            controlLimitedRadS2
        )
        >
        0.0001F
    )
    {
        saturationCounter++;
    }


    // ========================================================
    // CONVERSAO PARA MotorVelocity
    // ========================================================

    controlCommandDegS2 =
        controlLimitedRadS2
        *
        RAD_TO_DEG_LOCAL;


    // ========================================================
    // ESTATISTICAS DO CONTROLE
    // ========================================================

    const float absControlDegS2 =
        fabsf(
            controlCommandDegS2
        );


    if (
        absControlDegS2
        >
        maximumAbsControlDegS2
    )
    {
        maximumAbsControlDegS2 =
            absControlDegS2;
    }


    const float absPhiTerm =
        fabsf(
            controlPhiRadS2
            *
            RAD_TO_DEG_LOCAL
        );


    if (
        absPhiTerm
        >
        maximumAbsPhiTermDegS2
    )
    {
        maximumAbsPhiTermDegS2 =
            absPhiTerm;
    }


    const float absPhiDotTerm =
        fabsf(
            controlPhiDotRadS2
            *
            RAD_TO_DEG_LOCAL
        );


    if (
        absPhiDotTerm
        >
        maximumAbsPhiDotTermDegS2
    )
    {
        maximumAbsPhiDotTermDegS2 =
            absPhiDotTerm;
    }


    const float absBetaTerm =
        fabsf(
            controlBetaRadS2
            *
            RAD_TO_DEG_LOCAL
        );


    if (
        absBetaTerm
        >
        maximumAbsBetaTermDegS2
    )
    {
        maximumAbsBetaTermDegS2 =
            absBetaTerm;
    }


    const float absBetaDotTerm =
        fabsf(
            controlBetaDotRadS2
            *
            RAD_TO_DEG_LOCAL
        );


    if (
        absBetaDotTerm
        >
        maximumAbsBetaDotTermDegS2
    )
    {
        maximumAbsBetaDotTermDegS2 =
            absBetaDotTerm;
    }


    // ========================================================
    // MOTOR
    // ========================================================

    const bool accepted =
        motor.commandAcceleration(
            controlCommandDegS2,
            dtSeconds
        );


    if (
        !accepted
    )
    {
        endBalance(
            "MOTOR_COMMAND_REJECTED"
        );


        return;
    }


    motor.update();


    controlSampleCounter++;
}


// ============================================================
// ENCERRAMENTO DO BALANCE
// ============================================================
//
// Fazemos o snapshot ANTES de motor.stop().
//
// Isso corrige o problema anterior no qual:
//
//      PHIDOT_CMD_FINAL_DEG_S
//
// aparecia sempre como zero.
//
// ============================================================

void endBalance(
    const char *reason
)
{
    finalBalanceElapsedUs =
        micros()
        -
        balanceStartUs;


    finalPhiDeg =
        motor.currentPositionDegrees();


    finalPhiDotCommandDegS =
        motor.speedReferenceDegreesPerSecond();


    finalControlCommandDegS2 =
        controlCommandDegS2;


    motor.stop();


    printBalanceSummary(
        reason
    );


    clearControlVariables();


    controlState =
        ControlState::IDLE;


    captureStableStartUs =
        0;


    Serial.println(
        F("BALANCE encerrado.")
    );


    Serial.println(
        F("Motor continua habilitado e parado.")
    );


    Serial.println(
        F("Use D antes de reposicionar o braco manualmente.")
    );


    Serial.println();
}


// ============================================================
// STOP
// ============================================================

void stopControl()
{
    if (
        controlState
        ==
        ControlState::BALANCE
    )
    {
        endBalance(
            "USER_STOP"
        );


        return;
    }


    motor.stop();


    controlState =
        ControlState::IDLE;


    captureStableStartUs =
        0;


    clearControlVariables();


    Serial.println(
        F("Controle parado.")
    );
}


// ============================================================
// RESUMO
// ============================================================

void printBalanceSummary(
    const char *reason
)
{
    Serial.println();


    Serial.println(
        F("# BALANCE_END")
    );


    Serial.print(
        F("# REASON=")
    );

    Serial.println(
        reason
    );


    Serial.print(
        F("# TIME_MS=")
    );

    Serial.println(
        finalBalanceElapsedUs
        /
        1000UL
    );


    Serial.print(
        F("# SAMPLES=")
    );

    Serial.println(
        controlSampleCounter
    );


    Serial.print(
        F("# BETA_FINAL_DEG=")
    );

    Serial.println(
        betaRadians
        *
        RAD_TO_DEG_LOCAL,
        3
    );


    Serial.print(
        F("# BETADOT_FINAL_RAD_S=")
    );

    Serial.println(
        betaDotRadiansPerSecond,
        4
    );


    Serial.print(
        F("# PHI_FINAL_DEG=")
    );

    Serial.println(
        finalPhiDeg,
        3
    );


    Serial.print(
        F("# PHIDOT_CMD_FINAL_DEG_S=")
    );

    Serial.println(
        finalPhiDotCommandDegS,
        3
    );


    Serial.print(
        F("# U_FINAL_DEG_S2=")
    );

    Serial.println(
        finalControlCommandDegS2,
        2
    );


    Serial.print(
        F("# MAX_ABS_BETA_DEG=")
    );

    Serial.println(
        maximumAbsBetaRad
        *
        RAD_TO_DEG_LOCAL,
        3
    );


    Serial.print(
        F("# MAX_ABS_BETADOT_RAD_S=")
    );

    Serial.println(
        maximumAbsBetaDotRadS,
        4
    );


    Serial.print(
        F("# MAX_ABS_PHI_DEG=")
    );

    Serial.println(
        maximumAbsPhiDeg,
        3
    );


    Serial.print(
        F("# MAX_ABS_PHIDOT_CMD_DEG_S=")
    );

    Serial.println(
        maximumAbsPhiDotDegS,
        3
    );


    Serial.print(
        F("# MAX_ABS_U_DEG_S2=")
    );

    Serial.println(
        maximumAbsControlDegS2,
        1
    );


    Serial.println();


    Serial.println(
        F("# COMPONENTES DO CONTROLE")
    );


    Serial.print(
        F("# MAX_ABS_TERM_PHI_DEG_S2=")
    );

    Serial.println(
        maximumAbsPhiTermDegS2,
        1
    );


    Serial.print(
        F("# MAX_ABS_TERM_PHIDOT_DEG_S2=")
    );

    Serial.println(
        maximumAbsPhiDotTermDegS2,
        1
    );


    Serial.print(
        F("# MAX_ABS_TERM_BETA_DEG_S2=")
    );

    Serial.println(
        maximumAbsBetaTermDegS2,
        1
    );


    Serial.print(
        F("# MAX_ABS_TERM_BETADOT_DEG_S2=")
    );

    Serial.println(
        maximumAbsBetaDotTermDegS2,
        1
    );


    Serial.print(
        F("# SATURATION_COUNT=")
    );

    Serial.println(
        saturationCounter
    );


    Serial.println();
}


// ============================================================
// ARMAR BALANCE
// ============================================================

void armBalance()
{
    if (
        controlState
        !=
        ControlState::IDLE
    )
    {
        Serial.println(
            F("ERRO: sistema ocupado.")
        );


        return;
    }


    if (
        !pendulumDownReferenceDefined
    )
    {
        Serial.println(
            F("ERRO: calibre DOWN com T.")
        );


        return;
    }


    if (
        !armZeroDefined
    )
    {
        Serial.println(
            F("ERRO: defina phi=0 com Z.")
        );


        return;
    }


    if (
        !motor.isEnabled()
    )
    {
        Serial.println(
            F("ERRO: habilite o motor com E.")
        );


        return;
    }


    if (
        fabsf(
            motor.currentPositionDegrees()
        )
        >
        5.0F
    )
    {
        Serial.println(
            F("ERRO: braco deve iniciar perto de phi=0.")
        );


        return;
    }


    motor.stop();


    clearControlVariables();


    captureStableStartUs =
        0;


    controlState =
        ControlState::WAIT_CAPTURE;


    Serial.println();


    Serial.println(
        F("BALANCE FASE 09 ARMADO.")
    );


    Serial.println(
        F("Leve o pendulo perto da vertical.")
    );


    Serial.println();


    Serial.print(
        F("CAPTURE_READY se |beta| <= ")
    );

    Serial.print(
        CAPTURE_READY_BETA_DEG,
        2
    );

    Serial.println(
        F(" deg")
    );


    Serial.print(
        F("e |betaDot| <= ")
    );

    Serial.print(
        CAPTURE_READY_BETADOT_RAD_S,
        2
    );

    Serial.println(
        F(" rad/s")
    );


    Serial.print(
        F("por ")
    );

    Serial.print(
        CAPTURE_READY_TIME_US
        /
        1000UL
    );

    Serial.println(
        F(" ms.")
    );


    Serial.println();


    Serial.println(
        F("Quando aparecer CAPTURE_READY, solte.")
    );


    Serial.println();
}


// ============================================================
// REFERENCIA INFERIOR DO PENDULO
// ============================================================

void definePendulumDownReference()
{
    if (
        controlState
        !=
        ControlState::IDLE
    )
    {
        Serial.println(
            F("ERRO: pare o controle antes de T.")
        );


        return;
    }


    // A calibracao e feita com o motor desabilitado.

    if (
        motor.isEnabled()
    )
    {
        Serial.println(
            F("ERRO: desabilite o motor antes de T.")
        );


        return;
    }


    if (
        !pendulumPosition.magnetDetected()
    )
    {
        Serial.println(
            F("ERRO: ima nao detectado.")
        );


        return;
    }


    motor.stop();


    Serial.println();


    Serial.println(
        F("Calibrando referencia inferior...")
    );


    Serial.println(
        F("Pendulo deve estar LIVRE, PARADO e PARA BAIXO.")
    );


    // A biblioteca chama esta funcao de calibrateTop().
    //
    // Aqui deliberadamente usamos a posicao inferior
    // como referencia interna.
    //
    // O deslocamento de PI e realizado no main.cpp.

    pendulumPosition.calibrateTop(
        32,
        2
    );


    pendulumPosition.update();


    pendulumDownReferenceDefined =
        pendulumPosition.topIsDefined();


    if (
        !pendulumDownReferenceDefined
    )
    {
        Serial.println(
            F("ERRO ao definir referencia inferior.")
        );


        return;
    }


    betaRadians =
        calculateBetaFromDownReference();


    const uint32_t nowUs =
        micros();


    pendulumVelocity.reset(
        betaRadians,
        nowUs
    );


    betaDotRadiansPerSecond =
        0.0F;


    Serial.println();


    Serial.println(
        F("Referencia inferior definida.")
    );


    Serial.println(
        F("TOP = DOWN + 180 graus.")
    );


    Serial.print(
        F("beta atual [deg] ~= +/-180: ")
    );

    Serial.println(
        betaRadians
        *
        RAD_TO_DEG_LOCAL,
        3
    );


    Serial.println();
}


// ============================================================
// ZERO DO BRACO
// ============================================================

void defineArmZero()
{
    if (
        controlState
        !=
        ControlState::IDLE
    )
    {
        Serial.println(
            F("ERRO: sistema ocupado.")
        );


        return;
    }


    if (
        motor.isEnabled()
    )
    {
        Serial.println(
            F("ERRO: desabilite o motor antes de Z.")
        );


        return;
    }


    motor.setCurrentPosition(
        0.0F
    );


    armZeroDefined =
        true;


    Serial.println(
        F("Zero do braco definido: phi=0.")
    );
}


// ============================================================
// EMERGENCY STOP
// ============================================================

void emergencyStop()
{
    motor.emergencyStop();


    controlState =
        ControlState::IDLE;


    captureStableStartUs =
        0;


    clearControlVariables();


    armZeroDefined =
        false;


    Serial.println();


    Serial.println(
        F("EMERGENCY STOP.")
    );


    Serial.println(
        F("Motor desabilitado.")
    );


    Serial.println(
        F("Referencia do braco perdida.")
    );


    Serial.println();
}


// ============================================================
// SERIAL
// ============================================================

void serviceSerial()
{
    while (
        Serial.available()
        >
        0
    )
    {
        const char character =
            static_cast<char>(
                Serial.read()
            );


        if (
            character
            ==
            '\r'
        )
        {
            continue;
        }


        if (
            character
            ==
            '\n'
        )
        {
            if (
                serialBufferIndex
                >
                0
            )
            {
                serialBuffer[
                    serialBufferIndex
                ] =
                    '\0';


                handleCommand(
                    serialBuffer
                );


                serialBufferIndex =
                    0;
            }


            continue;
        }


        if (
            serialBufferIndex
            <
            SERIAL_BUFFER_SIZE
            -
            1
        )
        {
            serialBuffer[
                serialBufferIndex
            ] =
                character;


            serialBufferIndex++;
        }
    }
}


// ============================================================
// COMANDOS
// ============================================================

void handleCommand(
    char *command
)
{
    while (
        *command != '\0'
        &&
        isspace(
            static_cast<unsigned char>(
                *command
            )
        )
    )
    {
        command++;
    }


    for (
        char *p = command;
        *p != '\0';
        ++p
    )
    {
        *p =
            static_cast<char>(
                toupper(
                    static_cast<unsigned char>(
                        *p
                    )
                )
            );
    }


    // --------------------------------------------------------
    // Durante BALANCE evitamos comandos que possam gerar
    // impressao Serial longa e perturbar Ts.
    // --------------------------------------------------------

    if (
        controlState
        ==
        ControlState::BALANCE
    )
    {
        if (
            strcmp(command, "STOP") == 0
        )
        {
            stopControl();


            return;
        }


        if (
            strcmp(command, "X") == 0
        )
        {
            emergencyStop();


            return;
        }


        // Resposta curta para nao perturbar muito o loop.

        Serial.println(
            F("BALANCE ativo: use STOP ou X.")
        );


        return;
    }


    // --------------------------------------------------------
    // DOWN
    // --------------------------------------------------------

    if (
        strcmp(command, "T") == 0
        ||
        strcmp(command, "DOWN") == 0
    )
    {
        definePendulumDownReference();


        return;
    }


    // --------------------------------------------------------
    // ZERO
    // --------------------------------------------------------

    if (
        strcmp(command, "Z") == 0
        ||
        strcmp(command, "ZERO") == 0
    )
    {
        defineArmZero();


        return;
    }


    // --------------------------------------------------------
    // ENABLE
    // --------------------------------------------------------

    if (
        strcmp(command, "E") == 0
        ||
        strcmp(command, "ENABLE") == 0
    )
    {
        if (
            !armZeroDefined
        )
        {
            Serial.println(
                F("ERRO: defina Z primeiro.")
            );


            return;
        }


        motor.enable();


        Serial.println(
            F("Motor habilitado.")
        );


        return;
    }


    // --------------------------------------------------------
    // DISABLE
    // --------------------------------------------------------

    if (
        strcmp(command, "D") == 0
        ||
        strcmp(command, "DISABLE") == 0
    )
    {
        stopControl();


        motor.disable();


        // Sem encoder independente no braco.
        // Depois de desabilitar ele pode ser movimentado
        // manualmente e a referencia deixa de ser confiavel.

        armZeroDefined =
            false;


        Serial.println(
            F("Motor desabilitado.")
        );


        Serial.println(
            F("Referencia do braco perdida.")
        );


        return;
    }


    // --------------------------------------------------------
    // BALANCE
    // --------------------------------------------------------

    if (
        strcmp(command, "B") == 0
        ||
        strcmp(command, "BALANCE") == 0
    )
    {
        armBalance();


        return;
    }


    // --------------------------------------------------------
    // STOP
    // --------------------------------------------------------

    if (
        strcmp(command, "STOP") == 0
    )
    {
        stopControl();


        return;
    }


    // --------------------------------------------------------
    // EMERGENCY
    // --------------------------------------------------------

    if (
        strcmp(command, "X") == 0
    )
    {
        emergencyStop();


        return;
    }


    // --------------------------------------------------------
    // STATUS
    // --------------------------------------------------------

    if (
        strcmp(command, "S") == 0
        ||
        strcmp(command, "STATUS") == 0
    )
    {
        printStatus();


        return;
    }


    // --------------------------------------------------------
    // HELP
    // --------------------------------------------------------

    if (
        strcmp(command, "H") == 0
        ||
        strcmp(command, "HELP") == 0
        ||
        strcmp(command, "?") == 0
    )
    {
        printHelp();


        return;
    }


    Serial.println(
        F("Comando desconhecido. Use H.")
    );
}


// ============================================================
// STATUS
// ============================================================

void printStatus()
{
    pendulumPosition.update();


    if (
        pendulumDownReferenceDefined
    )
    {
        betaRadians =
            calculateBetaFromDownReference();
    }


    const float phiNow =
        motor.currentPositionDegrees();


    const float phiDotNow =
        motor.speedReferenceDegreesPerSecond();


    Serial.println();


    Serial.println(
        F("---------- STATUS ----------")
    );


    Serial.print(
        F("Estado: ")
    );

    Serial.println(
        stateName()
    );


    Serial.print(
        F("Referencia DOWN: ")
    );

    Serial.println(
        pendulumDownReferenceDefined
        ?
        F("DEFINIDA")
        :
        F("NAO DEFINIDA")
    );


    Serial.print(
        F("Zero braco: ")
    );

    Serial.println(
        armZeroDefined
        ?
        F("DEFINIDO")
        :
        F("NAO DEFINIDO")
    );


    Serial.print(
        F("Motor: ")
    );

    Serial.println(
        motor.isEnabled()
        ?
        F("HABILITADO")
        :
        F("DESABILITADO")
    );


    Serial.print(
        F("beta [deg]: ")
    );

    Serial.println(
        betaRadians
        *
        RAD_TO_DEG_LOCAL,
        3
    );


    Serial.print(
        F("betaDot [rad/s]: ")
    );

    Serial.println(
        betaDotRadiansPerSecond,
        4
    );


    Serial.print(
        F("phi [deg]: ")
    );

    Serial.println(
        phiNow,
        3
    );


    Serial.print(
        F("phiDot cmd [deg/s]: ")
    );

    Serial.println(
        phiDotNow,
        3
    );


    Serial.print(
        F("u [deg/s2]: ")
    );

    Serial.println(
        controlCommandDegS2,
        2
    );


    Serial.println(
        F("----------------------------")
    );


    Serial.println();
}


// ============================================================
// NOME DO ESTADO
// ============================================================

const char *stateName()
{
    switch (
        controlState
    )
    {
        case ControlState::IDLE:
        {
            return "IDLE";
        }


        case ControlState::WAIT_CAPTURE:
        {
            return "WAIT_CAPTURE";
        }


        case ControlState::WAIT_RELEASE:
        {
            return "WAIT_RELEASE";
        }


        case ControlState::BALANCE:
        {
            return "BALANCE";
        }
    }


    return "UNKNOWN";
}


// ============================================================
// HELP
// ============================================================

void printHelp()
{
    Serial.println(
        F("COMANDOS:")
    );


    Serial.println(
        F("T       = calibrar pendulo PARA BAIXO")
    );


    Serial.println(
        F("Z       = definir phi=0")
    );


    Serial.println(
        F("E       = habilitar motor")
    );


    Serial.println(
        F("D       = desabilitar motor")
    );


    Serial.println(
        F("B       = armar captura + BALANCE")
    );


    Serial.println(
        F("STOP    = parar controle")
    );


    Serial.println(
        F("X       = emergency stop")
    );


    Serial.println(
        F("S       = status")
    );


    Serial.println(
        F("H       = ajuda")
    );


    Serial.println();


    Serial.println(
        F("SEQUENCIA:")
    );


    Serial.println(
        F("1. Centralize fisicamente o braco.")
    );


    Serial.println(
        F("2. D")
    );


    Serial.println(
        F("3. Z")
    );


    Serial.println(
        F("4. Pendulo livre e parado para baixo.")
    );


    Serial.println(
        F("5. T")
    );


    Serial.println(
        F("6. E")
    );


    Serial.println(
        F("7. B")
    );


    Serial.println(
        F("8. Leve o pendulo ao topo.")
    );


    Serial.println(
        F("9. Aguarde CAPTURE_READY e solte.")
    );


    Serial.println();


    Serial.println(
        F("CONTROLE DE 4 ESTADOS:")
    );


    Serial.print(
        F("Kphi = ")
    );

    Serial.println(
        K_PHI,
        5
    );


    Serial.print(
        F("KphiDot = ")
    );

    Serial.println(
        K_PHI_DOT,
        5
    );


    Serial.print(
        F("Kbeta = ")
    );

    Serial.println(
        K_BETA,
        5
    );


    Serial.print(
        F("KbetaDot = ")
    );

    Serial.println(
        K_BETA_DOT,
        5
    );


    Serial.println();


    Serial.println(
        F("Abort beta = +/-8 deg.")
    );


    Serial.println(
        F("Abort phi = +/-75 deg.")
    );


    Serial.println(
        F("u max = +/-600 deg/s2.")
    );


    Serial.println(
        F("Teste maximo = 5 s.")
    );


    Serial.println();
}